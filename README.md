...
const getPosts = (searchObject, sortObject, limit, skip) => {
    const posts = db.blogs.find(searchObject).sort(sortObject).limit(limit).skip(skip).toArray()//***STRETCH GOAL****added .sort(), .limit(), .skip(), for Stretch Goals 
    return posts
}

// const searchObject = {createdAt: {$gt: new Date("05/01/2022")}} 
const searchObject = {title: "The Dark Knight Movie Review"} //***STRETCH GOAL**** using getPosts() to update one 
...
console.log(posts)
// console.log(posts[0].id) //***STRETCH GOAL****used this console log for stretch foal for updateOne problem
...
const updatePost = (id, newPostData) => {
    db.blogs.updateOne({
        ...
        $set: {
            ...
            // author: newPostData.author,//***Stretch Goals***Comment out both line to verify stretch goals are correct
            lastModified: new Date() 
        }
...
// // const postId = getPosts(searchObject, sortObject, limit, skip)[0].id//***Stretch Goal***
